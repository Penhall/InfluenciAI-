...
- POST `/auth/login` - retorna `{ access_token, refresh_token }`; body: `{ email, password }` - livre
  - POST `/auth/refresh` - troca `refresh_token` por novos tokens; body: `{ refresh_token }`; retorna `{ access_token, refresh_token }`
  - POST `/auth/logout` - revoga um `refresh_token`; body: `{ refresh_token }` (requer Authorization)
  - POST `/auth/logout/all` - revoga todos os refresh tokens do usuário atual (requer Authorization)
